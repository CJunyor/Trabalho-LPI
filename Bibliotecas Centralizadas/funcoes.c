...
int Cadastro(int type, Acervo *p){
    ...
    pf=fopen("Acervos.bin","a+");
    if(pf==NULL) return 0;
    fwrite(p, sizeof(Acervo),1,pf);
    fclose(pf);
    return 1;
...
int Consultar(char *str, int TConsu, Acervo *p){
    int i;
    if(TConsu==1){
        for(i=0;i<TAMMAX&&(p[i].type==1||p[i].type==2);i++){
            if(p[i].type==1){
                    if(strcmp(str,(p[i].L.Nome))==0){
                        switch(p[i].L.status){
                            case 1 :printf("STATUS : EMPRESTADO\n");
                                break;
                            case 2 :printf("STATUS : DISPONIVEL\n");
                                break;
                            case 3:printf("STATUS : RESERVADO\n");
                                break;
                        }
                        if(p[i].L.ExemplarP)
                            printf("EXEMPLAR : SIM\n");
                    ...
                    if(strcmp(str,(p[i].P.Titulo))==0){
                        ...
                        printf("SECAO : %s\n",p[i].P.Localizacao.Secao);
                        printf("ESTANTE : %s\n",p[i].P.Localizacao.Estante);
                        printf("PRATELEIRA : %s\n",p[i].P.Localizacao.Prateleira);
                        return 1;
                    }
            }
        }
